...
`generation_mode` 只作为输出表单“确认生图”按钮触发后的内部执行状态，不出现在用户输入表单。
首轮必须先输出 `master_prompt_markdown`，并将图片状态标记为 `awaiting_confirmation`。
只有用户点“确认生图”或输入等价确认后，才切换为 `generate_image` 并返回图片结果或结构化图片错误。
